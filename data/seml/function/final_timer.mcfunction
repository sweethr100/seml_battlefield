schedule function seml:final_timer 1s
...
bossbar set minecraft:time name [{"text":"최후의 전장까지 남은 시간 : "},{"score":{"name":"bossbar_min","objective":"timer"}},{"text":"분 "},{"score":{"name":"bossbar_sec","objective":"timer"}},{"text":"초"}]
bossbar set minecraft:time color white

execute if score bossbar_min timer matches 0 if score bossbar_sec timer matches 0 run function seml:tofinal with storage seml:data
